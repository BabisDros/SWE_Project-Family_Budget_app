...
public class GlobalStatisticsPresenterTest {

    private Initializer memory;
    private FamilyDAO familyDAO;

    private GlobalStatisticsPresenter presenter;

    private final int yearlyStat1 = 1000;
    private final int yearlyStat2 = 5000;
    private final int yearlyStat3 = 10000;
    @Before
    public void setUp(){
        memory = new MemoryInitializer();
        memory.prepareData();
        familyDAO = new FamilyDAOMemory();
        presenter = new GlobalStatisticsPresenter(familyDAO);
    }

    /**
     * Test whether code refactor will have the same result as the previous defined code
     */
    @Test
    public void getMonthlyStatCodeChanges() {
        List<Tuples<YearMonth, Double>> expected = getMonthlyStat();
        List<Tuples<YearMonth, Double>> result = presenter.getMonthlyStat();
     ...
     */
    @Test
    public void getYearlyStatCodeChanges() {
        List<Quadruples<YearMonth, Double, Double, Double>> expected = getYearlyStat();
        List<Quadruples<YearMonth, Double, Double, Double>> result = presenter.getYearlyStat();
    ...
    /**
     * This method is known to be giving the correct results of {@code Monthly} statistics
     * and its being used as a comparison metric
     */
    ...
    /**
     * This method is known to be giving the correct results of {@code Yearly} statistics
     * and its being used as a comparison metric
     */
